...
docNode* docList::Search(int &doc)
{
	docNode * p = head;
	while (p != nullptr)
	{
...
bool docList::Edit(int &doc, int &times)
{
	docNode * p = Search(doc);
	if (p != nullptr)
	{
...
void docList::output(fstream &out)//输出链表内容
{
	docNode *p = head;
	while (p != nullptr)
	{
...
void docList::merge(docList *list)
{
